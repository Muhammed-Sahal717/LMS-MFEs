...
export function AppFrame({ children }: { children: ReactNode }) {
  ...
  const fullPath = "/learning" + (pathname === "/" ? "" : pathname);
  ...
  const actions = loading ? (
    ...
    <div className="flex items-center gap-2">
      <ThemeToggle />
      <a href="/auth/login"><Button size="sm">Sign in</Button></a>
    </div>
  );

  return (
    <AppShell activeHref={fullPath} can={can} hasModule={hasModule} userRoles={user?.roles?.map(r => r.code) || []} actions={actions}>
      {children}
    </AppShell>
  );
}
